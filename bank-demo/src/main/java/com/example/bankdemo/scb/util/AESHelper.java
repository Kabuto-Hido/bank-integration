package com.example.bankdemo.scb.util;
...
public class AESHelper {
    ...
    public <T> T decryptObject(String value, TypeReference<T> typeReference) {
        try {
            ObjectMapper mapper = new Jackson2ObjectMapperBuilder().build();
            mapper.enable(DeserializationFeature.ACCEPT_EMPTY_STRING_AS_NULL_OBJECT);
            String decryptString = decrypt(value);
            return mapper.readValue(decryptString, typeReference);
        } catch (Exception e) {
            throw new IllegalArgumentException(e.getMessage());
        }
    }
